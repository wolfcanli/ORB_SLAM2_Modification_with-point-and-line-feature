...
void Optimizer::GlobalBundleAdjustemntWithLine(ORB_SLAM2::Map *pMap, int nIterations, bool *pbStopFlag,
...
*/
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMPs,
 ...
 *         = (u, v)-project(Tcw*Pw)
 *              + line * project(Tcw*LineStartPointW) + line * project(Tcw*LineEndPointW)
 * 1. Vertex: g2o::VertexSE3Expmap()，即当前帧的Tcw
 *            g2o::VertexSBAPointXYZ()，MapPoint的mWorldPos
 *            g2o::VertexLinePointXYZ(), MapLine的端点的世界坐标
 * 2. Edge:
 *     - g2o::EdgeSE3ProjectXYZ()，BaseBinaryEdge
 *         + Vertex：待优化当前帧的Tcw
 *         + Vertex：待优化MapPoint的mWorldPos
 ...
 *          bRobust  是否使用核函数
 */
void Optimizer::BundleAdjustmentWithLines(const std::vector<KeyFrame*> &vpKFs,
                                          const std::vector<MapPoint*> &vpMPs,
                                          const std::vector<MapLine*> &vpMLs,
                                          int nIterations,
                                          bool *pbStopFlag,
    ...
    vbNotIncludedLineSP.resize(vpMLs.size());

    // Step 2.3：向优化器添加地图线作为顶点
    // 先设置线段的起始点
...
//            cout << "MapLine StartPoint Id = " << ids << endl;
        ...
        for(map<KeyFrame*, size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            line_obs = pKF->mvKeyLineCoefficient[mit->second];  ///
...
// -----------------------------------------------------------------
    ...
    vbNotIncludedLineEP.resize(vpMLs.size());
...
//        VertexLinePointXYZ* vEndP = new VertexLinePointXYZ();     //自定义的顶点
        ...
        const int ide = pML->mnId + maxKFid + maxMapPointID + vpMLs.size() + 1;
...
//        VertexLinePointXYZ* vEndP = static_cast<VertexLinePointXYZ*>(optimizer.vertex(2*pML->mnId + maxKFid + vpMP.size() + 1));  //自定义的顶点
        ...
        g2o::VertexSBAPointXYZ* vEndP = static_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(pML->mnId + maxKFid + maxMapPointID + vpMLs.size() + 1));
        //todo 如果更改了，序号也要对应起来
...
int Optimizer::PoseOptimizationWithLines(Frame* pFrame) {
    ...
    // 如果没有足够的匹配点,那么就只好放弃了
    if(nInitialCorrespondences < 3 && nLineInitialCorrespondences < 3)
        return 0;

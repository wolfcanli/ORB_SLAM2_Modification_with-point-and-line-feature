...
void LocalMapping::CreateNewMapLines() {
    ...
    LineMatcher line_matcher;    //建立线特征匹配
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++) {
        ...
        vector<pair<size_t, size_t>> vMatchedIndices;
        line_matcher.SearchForTriangulation(mpCurrentKeyFrame, pKF2, vMatchedIndices, false);
        ...
        for(int ikl=0; ikl<nmatches; ikl++)
        {
            ...
            const Vector3d keyline1_function = mpCurrentKeyFrame->mvKeyLineCoefficient[idx1];
            const Vector3d keyline2_function = pKF2->mvKeyLineCoefficient[idx2];
            const cv::Mat klF1 = (cv::Mat_<float>(3,1) << keyline1_function(0),
                                                          keyline1_function(1),
                                                          keyline1_function(2));
            const cv::Mat klF2 = (cv::Mat_<float>(3,1) << keyline2_function(0),
                                                          keyline2_function(1),
                                                          keyline2_function(2));

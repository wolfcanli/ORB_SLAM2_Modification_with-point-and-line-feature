...
{
    cv::Mat im; // 也是outImage
    vector<cv::KeyPoint> vIniKeys; // Initialization: KeyPoints in reference frame
    std::vector<KeyLine> vIniKeyLines; // Initialization: KeyLines in reference frame

    vector<int> vMatches; // Initialization: correspondeces with reference keypoints
    vector<int> vLineMatches; // Initialization: correspondeces with reference keylines

    vector<cv::KeyPoint> vCurrentKeys; // KeyPoints in current frame
    vector<KeyLine> vCurrentKeyLines; // KeyLines in current frame
    ...
    //NOTICE 加互斥锁，避免与FrameDrawer::Update函数中图像拷贝发生冲突
    {
        ...
        if(mState==Tracking::NOT_INITIALIZED) {
            ...
            vCurrentKeyLines = mvCurrentKeyLine;

            vIniKeys = mvIniKeys;
            vIniKeyLines = mvIniKeyLines;

            vMatches = mvIniMatches;
            vLineMatches = mvIniLineMatches;
...
//        // Draw KeyLines
//        std::vector<KeyLine> keyline_in_map;
//        std::vector<KeyLine> keyline_in_vo;
//
...
//                if(vbLineMap[i]){
//                    // 用红色表示地图中的
//                    keyline_in_map.push_back(vCurrentKeyLines[i]);
//                    mnLineTracked++;
//                } else {
//                    // 仅当前帧观测到的用蓝色表示
//                    keyline_in_vo.push_back(vCurrentKeyLines[i]);
//                    mnLineTrackedVO++;
//                }
//            }
//        }
//
//        // 用红色表示地图中的
//        drawKeylines(im, keyline_in_map, im, cv::Scalar(0, 0, 255));
//        // 仅当前帧观测到的用蓝色表示
//        drawKeylines(im, keyline_in_vo, im, cv::Scalar(255, 0, 0));
        drawKeylines(im, vCurrentKeyLines, im, cv::Scalar(0, 0, 255));
...
void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText)
{
    ...
    else if(nState==Tracking::OK)
    {
        ...
        s << "KFs: " << nKFs << ", MPs: " << nMPs << ", MLs: " << nMLs << ", Matches: " << mnTracked;
        //在视觉里程计中匹配到的
        if(mnTrackedVO>0)
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    // 拷贝跟踪线程的特征线
    mvCurrentKeyLine = pTracker->mCurrentFrame.mvKeyLines;
    NL = mvCurrentKeyLine.size();
    mvbLineVO = std::vector<bool>(NL, false);
    mvbLineMap = std::vector<bool>(NL, false);
